...
// src/assets/andrewsolomon.pdf and src/assets/andrewsolomon-light.pdf from
// ../main.pdf and ../main-light.pdf so the Export-PDF link always serves the
// latest LaTeX build for both theme variants.
...
const upstreamLightPdf = resolve(__dirname, '../../main-light.pdf');
const bundledPdf = resolve(__dirname, '../src/assets/andrewsolomon.pdf');
const bundledLightPdf = resolve(__dirname, '../src/assets/andrewsolomon-light.pdf');
...
if (existsSync(upstreamLightPdf)) {
  copyFileSync(upstreamLightPdf, bundledLightPdf);
  console.log(`sync: copied ${upstreamLightPdf} -> ${bundledLightPdf}`);
} else {
  console.log('sync: ../main-light.pdf not built yet; leaving src/assets/andrewsolomon-light.pdf as committed.');
}
